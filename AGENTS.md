...
- Root `typography.css` is the shared font and font-size configuration for the
  ...
  the heading/label face, and Fira Code the code face. Root `typography.js` drives
  the decks' Aa controls for text/math sizing and per-deck browser preferences.
  Keep range-input keyboard and touch gestures independent of slide navigation.
  Meow's figure script reads the same font URLs and global font/figure scales through
...
repository root so each deck can load `typography.css` and `typography.js`. To preview the
