(() => {
  ...
  function updateHud() {
    ...
    chapter.textContent = slide.dataset.chapter || "meow";
    progress.style.width = `${((current + 1) / slides.length) * 100}%`;
    document.title = `${titleFor(slide)} — meow`;
  }
  ...
  function buildOverview() {
    ...
    slides.forEach((slide, index) => {
      ...
      button.innerHTML = `<span class="n">${String(index + 1).padStart(2, "0")} · ${slide.dataset.chapter || "meow"}</span><strong>${titleFor(slide)}</strong><span>${slide.dataset.summary || ""}</span>`;
